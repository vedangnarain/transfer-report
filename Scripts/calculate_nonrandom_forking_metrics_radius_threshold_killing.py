...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct 21 22:32:20 2021

@author: narain

Tested in Python 3.7.4.

Calculate metrics of Nonrandom Forking Network with radius threshold pruning. The metrics are: 
...
def get_distribution_stats(solver_name, alpha_value, radius_threshold, hypoxic_threshold_list, plot=0, read=0):    
    
    # Set the file path
    folder_path = '/home/narain/Desktop/Results/Forking/Nonrandom/Radius Threshold Pruning in Nonrandom Forking Network/TestDichotomousNetwork/' + solver_name + 'Haematocrit/Lambda4/Alpha' + alpha_value + '/RadiusThreshold' + radius_threshold
...
def get_solver_stats(solver_name, alpha_list, radius_threshold_list, hypoxic_threshold_list, plot, read):
#    table = np.array([])
    alpha_table = np.array([])
    for alpha_value in alpha_list:
        for radius_threshold in radius_threshold_list:    
            average_radius_threshold_data = get_distribution_stats(solver_name, alpha_value, radius_threshold, hypoxic_threshold_list, plot, read)
            table_entry = np.hstack([float(alpha_value), float(radius_threshold), average_radius_threshold_data])
...
solver_stats = get_solver_stats(solver_name, alpha_list, radius_threshold_list, hypoxic_threshold_list, plot=0, read=0)

# Save array
#np.save('/home/narain/Desktop/Results/Forking/Nonrandom/Radius Threshold Pruning in Nonrandom Forking Network/TestDichotomousNetwork/' + solver_name + 'Haematocrit/python_solver_data.npy', solver_stats)
#solver_stats = np.load('/home/narain/Desktop/Results/Forking/Nonrandom/Radius Threshold Pruning in Nonrandom Forking Network/TestDichotomousNetwork/' + solver_name + 'Haematocrit/python_solver_data.npy')
...
# Read PQ file
#filename = '/home/narain/Desktop/Results/Forking/Nonrandom/Radius Threshold Pruning in Nonrandom Forking Network/TestDichotomousNetwork/forking_radius_threshold_perfusion_quotients.txt'
filename = '/home/narain/Chaste/forking_radius_threshold_perfusion_quotients.txt'
...
for i in range(len(alpha_list),len(alpha_list)*2):
    ...
    axs[i].plot(alpha_array[:,1], pq_composite[i-5,:], label='PQ')
    axs[i].set_xlim(0)
    axs[i].set_ylim(0)
#    axs[i].set_xlabel('radius threshold (μm)')    
    if i==len(alpha_list):
...
for i in range(len(alpha_list*2),len(alpha_list)*3):
    ...
    unpacked_hypoxic_thresholds = np.concatenate( hypoxic_fraction_composite[offset*len(radius_threshold_list):(offset+1)*len(radius_threshold_list)], axis=0 )
    for threshold_index in range(len(hypoxic_threshold_list)):  # plot a line for each threshold
        single_hypoxic_threshold = [item[threshold_index] for item in unpacked_hypoxic_thresholds]
        axs[i].plot(alpha_array[:,1], single_hypoxic_threshold, label=linelegends[threshold_index], ls=linestyles[threshold_index], c=linecolours[threshold_index])
...
# Save image
file_path = Path('~/Desktop/Final Figures/' + solver_name + '_nonrandom_forking_radius_threshold_pruning.svg').expanduser()
fig.savefig(file_path, dpi=500, bbox_inches = 'tight')
file_path = Path('~/Desktop/Final Figures/' + solver_name + '_nonrandom_forking_radius_threshold_pruning.png').expanduser()
fig.savefig(file_path, dpi=500, bbox_inches = 'tight')
