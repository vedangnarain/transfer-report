...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct 14 21:38:45 2021
...
stochastic pruning. The metrics are: basic distribution stats, the perfusion 
...
def get_distribution_stats(solver_name, layout_selection, sigma, beta, hypoxic_threshold_list, plot=0, read=0):    
...
#    folder_path = '/home/narain/Desktop/Stochastic Pruning with 100 Trials/' + solver_name + 'Haematocrit/Lambda4/Alpha' + alpha_value + '/Beta' + beta + '/Trial' + trial
    folder_path = '/scratch/narain/Hexagonal/Log Normal Distribution/Stochastic Pruning in Hexagonal Network with 100 Selections/TestHexagonalNetwork/' + solver_name + 'Haematocrit/Sigma' + sigma + '/Selection' + layout_selection + '/Beta' + beta
...
#    network_path = '/home/narain/Desktop/ReferenceNetworks/Alpha' + alpha_value + '/FinalHaematocrit.vtp'
    ...
    if read==0:    
        ...
        if plot==1:  # plot the O2 distribution
            ...
            plt.suptitle('O$_2$ distribution generated by the ' + solver_name + ' solver in the hexagonal vessel network with radii selection = ' + layout_selection + ' (σ = ' + sigma + ' and beta = ' + beta + '  vessels (1 unit = ' + str(field_spacing [0]) + ' μm)')
...
def compute_average_beta(solver_name, sigma, beta, max_layouts, hypoxic_threshold_list, plot, read):
...
#        print(layout_selection)
        hypoxic_fraction_list, mean_value, min_value, half_value, max_value, std_value = get_distribution_stats(solver_name, str(layout_selection), sigma, str(beta), hypoxic_threshold_list, plot, read)
...
def get_solver_stats(solver_name, alpha_list, beta_list, max_layouts, hypoxic_threshold_list, plot, read):
#    table = np.array([])
    alpha_table = np.array([])
    for sigma in alpha_list:
        for beta in beta_list:    
            average_beta_data = compute_average_beta(solver_name, sigma, beta, max_layouts, hypoxic_threshold_list, plot, read)
            table_entry = np.hstack([float(sigma), float(beta), average_beta_data])
...
#solver_stats = get_solver_stats(solver_name, alpha_list, beta_list, max_layouts, hypoxic_threshold_list, plot=0, read=0)

# Save array
#np.save('/scratch/narain/Hexagonal/Log Normal Distribution/Stochastic Pruning in Hexagonal Network with 100 Selections/TestHexagonalNetwork/' + solver_name + 'Haematocrit/python_solver_data.npy', solver_stats)
solver_stats = np.load('/scratch/narain/Hexagonal/Log Normal Distribution/Stochastic Pruning in Hexagonal Network with 100 Selections/TestHexagonalNetwork/' + solver_name + 'Haematocrit/python_solver_data.npy')
...
# Read PQ file
filename = '/scratch/narain/Hexagonal/Log Normal Distribution/Stochastic Pruning in Hexagonal Network with 100 Selections/TestHexagonalNetwork/hex_lognormal_stochastic_perfusion_quotients.txt'
...
# Save image
file_path = Path('~/Desktop/Final Figures/' + solver_name + '_lognormal_hexagonal_stochastic_pruning.svg').expanduser()
fig.savefig(file_path, dpi=500, bbox_inches = 'tight')
file_path = Path('~/Desktop/Final Figures/' + solver_name + '_lognormal_hexagonal_stochastic_pruning.png').expanduser()
fig.savefig(file_path, dpi=500, bbox_inches = 'tight')
